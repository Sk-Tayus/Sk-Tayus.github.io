...
if (showData) {
  // Example key: MOVIE_LANG_DATE_THEATRE_TIME
...
document.body.addEventListener("click", (e) => {
  ...
  // UNSELECT SEAT
    ...
    // when unselect, mark as available (we don't keep bestseller state here)
...
/* ------------------------------------------
   PAY BUTTON → LOCK SEATS & GO TO FOOD & MERCH
------------------------------------------- */
...
payBtn.addEventListener("click", () => {
  ...
  // Save seat selection summary for food page
  ...
  localStorage.setItem("finalBookingSummary", JSON.stringify(finalBookingSummary));

  // ⭐ Redirect to Food & Merch
...
/* ------------------------------------------
   TEST PAYMENT: ₹1 PAYMENT QUICK TEST
------------------------------------------- */
...
document.getElementById("testPayBtn").addEventListener("click", () => {
  ...
  localStorage.setItem("finalOrderSummary", JSON.stringify(testBooking));

  alert("Redirecting to test payment (₹1)...");
